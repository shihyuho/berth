...
With the settings window open in each language, verify that selecting **Settings…** again focuses the same window, changing the Pinned Display updates both settings and the menu, and display or Accessibility changes refresh without reopening the window. Exercise both the initial Accessibility grant and revoked-permission recovery routes. Toggle Launch at Login in settings, confirm the reported registration state changes, then restore the original state. Confirm the About section shows the packaged Berth version and opens the project page.

For update checks, verify the menu and Settings actions report current, available, and failed outcomes; disabling automatic checks leaves manual checks available; dismissing an automatic update prompt does not repeat it for the same release; and the available version remains visible in both surfaces. Confirm **View Update Instructions…** opens the guide with separate direct-download and Homebrew paths. Verify no update is downloaded or installed and macOS Notification Center permission is never requested.
