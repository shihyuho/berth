...
請在兩種語言下開啟設定視窗，確認再次選擇「設定…」會聚焦同一個視窗；變更固定螢幕後，設定與選單會同步更新；顯示器或「輔助使用」狀態改變時，不必重開視窗也會刷新。請分別演練初次授權與權限遭撤銷後的恢復路徑。在設定中切換「登入時自動啟動」，確認顯示的登錄狀態會更新，再恢復原本狀態。確認「關於」區段顯示封裝 App 的 Berth 版本，並能開啟專案頁面。

更新檢查需驗證：選單與「設定」的動作都能顯示目前為最新、已有更新及檢查失敗；停用自動檢查後仍可手動檢查；關閉自動更新提示後，同一版本不會重複提示；選單與「設定」仍會顯示可用版本。確認「查看更新說明…」會開啟分列直接下載與 Homebrew 流程的指南，而且 Berth 不會下載或安裝更新，也不會要求 macOS 通知中心權限。

在「一般」區段，確認「App 語言」顯示目前 Berth process 使用的語言。請分別在「系統設定」關閉及已開啟於其他面板時，選擇「開啟『語言與地區』…」。在「應用程式」區塊將 Berth 設為另一個支援的語言、重新開啟 Berth，並確認完整 UI 與 Effective Language 一致。移除 App 專屬的語言覆寫、重新開啟 Berth，確認 Berth 重新跟隨 macOS 語言偏好。請在 macOS 13 與目前支援的最新版本演練此 handoff。若「語言與地區」路徑與「系統設定」fallback 都無法開啟，確認 Berth 顯示在地化的手動路徑。面板 deep link 並非公開的第三方契約，因此請在 PR 中將任何尚未驗證的 macOS 版本列為待補證據。
