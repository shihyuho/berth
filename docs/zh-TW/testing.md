...
請在兩種語言下開啟設定視窗，確認再次選擇「設定…」會聚焦同一個視窗；變更固定螢幕後，設定與選單會同步更新；顯示器或「輔助使用」狀態改變時，不必重開視窗也會刷新。請分別演練初次授權與權限遭撤銷後的恢復路徑。在設定中切換「登入時自動啟動」，確認顯示的登錄狀態會更新，再恢復原本狀態。確認「關於」區段顯示封裝 App 的 Berth 版本，並能開啟專案頁面。畫面中不應出現任何自動更新控制項。
